...
async def test_facility_logic():
    ...
    async with TestingSessionLocal() as db:
        try:
            ...
            asset = await asset_service.create_asset(db, asset_data)
            print(f"Asset created: {asset.name} (ID: {asset.id})")
            assert asset.name == "Aire Acondicionado LG"
            
            assets = await asset_service.list_assets(db, prop.id)
            assert len(assets) == 1
            ...
            insp = await inspection_service.create_inspection(db, insp_data)
            print(f"Inspection created: {insp.inspection_type} (ID: {insp.id})")
            assert insp.inspection_type == "Preventiva"
            
            inspections = await inspection_service.list_inspections(db, prop.id)
...
if __name__ == "__main__":
    asyncio.run(test_facility_logic())
