...
async def list_inventories(db: AsyncSession, property_id: str):
    stmt = select(PropertyInventory).where(PropertyInventory.property_id == property_id).order_by(PropertyInventory.date.desc())
    result = await db.execute(stmt)
    return result.scalars().all()

async def get_inventory(db: AsyncSession, inventory_id: str):
    stmt = select(PropertyInventory).where(PropertyInventory.id == inventory_id)
    result = await db.execute(stmt)
